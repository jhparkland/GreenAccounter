...
from VGGNet.DB_Module import FireBase
...
class GPUs:
    ...
    def upload_resource(self): # Firebase에 현재 Resource Upload
        ...
        self._fb.upload_current_resource(round(cpus, 2), round(memorys, 2), 
                                         round(total_memorys, 2), gpus, sum(self.used_gpu), 
                                         epoch, clock, max_clock, learning_time+self.learned_time, 
                                         sum(self.carbon_emissions), self._carbons)
